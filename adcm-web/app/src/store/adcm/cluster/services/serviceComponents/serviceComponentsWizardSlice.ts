import { AdcmClusterMappingApi, AdcmClusterServiceComponentsApi, AdcmJobsApi, type RequestError } from '@api';
...
const getProcess = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/getProcess',
  async ({ clusterId, serviceId, componentId, actionId, processId }: AdcmGetProcessPayload, thunkAPI) => {
    try {
      const process = await AdcmClusterServiceComponentsApi.getClusterServiceComponentActionWizardProcess(
        clusterId,
        serviceId,
        componentId,
        actionId,
...
const getProcessOnActionClick = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/getProcessOnActionClick',
  async ({ clusterId, serviceId, componentId, actionId, processId }: AdcmGetProcessPayload, thunkAPI) => {
    try {
      const process = await AdcmClusterServiceComponentsApi.getClusterServiceComponentActionWizardProcess(
        clusterId,
...
const refreshProcessStages = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/refreshProcessStages',
  async ({ clusterId, serviceId, componentId, actionId, processId }: AdcmGetProcessPayload, thunkAPI) => {
    try {
      const process = await AdcmClusterServiceComponentsApi.getClusterServiceComponentActionWizardProcess(
        clusterId,
...
const getStep = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/getStep',
  async ({ clusterId, serviceId, componentId, actionId, processId, stepId }: AdcmGetStepPayload, thunkAPI) => {
    try {
      const step = await AdcmClusterServiceComponentsApi.getClusterServiceComponentActionWizardStep(
        clusterId,
        serviceId,
        componentId,
        actionId,
...
const getSteps = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/getSteps',
  async ({ clusterId, serviceId, componentId, actionId, processId, stepIds }: AdcmGetStepsPayload, thunkAPI) => {
    try {
      ...
      const stepPromises = stepIds.map((stepId) =>
        AdcmClusterServiceComponentsApi.getClusterServiceComponentActionWizardStep(
          clusterId,
          serviceId,
          componentId,
          actionId,
...
const loadJobFromBackend = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/loadJobFromBackend',
...
const getJob = createAsyncThunk('adcm/clusterServiceComponentsWizard/getJob', async (arg: loadJobPayload, thunkAPI) => {
...
const loadSubJobLogFromBackend = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/loadSubJobLogFromBackend',
...
type GetClustersServiceComponentsWizardMappingArg = {
  clusterId: number;
};

const loadMappings = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/loadMappings',
  async ({ clusterId }: GetClustersServiceComponentsWizardMappingArg, thunkAPI) => {
    try {
...
const getMappings = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/getMappings',
  async (arg: GetClustersServiceComponentsWizardMappingArg, thunkAPI) => {
    await thunkAPI.dispatch(loadMappings(arg));
  },
);

const refreshMapping = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/mapping/refreshMapping',
  async ({ clusterId }: GetClustersServiceComponentsWizardMappingArg, _thunkAPI) => {
    const mapping = await AdcmClusterMappingApi.getMapping(clusterId);
    return mapping;
  },
);
...
const refreshMappingHosts = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/mapping/refreshMappingHosts',
  async ({ clusterId }: GetClustersServiceComponentsWizardMappingArg, _thunkAPI) => {
    const hosts = await AdcmClusterMappingApi.getMappingHosts(clusterId);
    return hosts;
  },
...
const refreshMappingComponents = createAsyncThunk(
  'adcm/clusterServiceComponentsWizard/mapping/refreshMappingComponents',
  async ({ clusterId }: GetClustersServiceComponentsWizardMappingArg, _thunkAPI) => {
    const components = await AdcmClusterMappingApi.getMappingComponents(clusterId);
    return components;
  },
...
const clusterServiceComponentsWizardSlice = createSlice({
  name: 'adcm/clusterServiceComponentsWizard',
...
export const {
  ...
  resetJobData,
  resetJobDataByStep,
} = clusterServiceComponentsWizardSlice.actions;
export {
  getStep,
  getSteps,
  getProcess,
...
export default clusterServiceComponentsWizardSlice.reducer;

...
import { AdcmClusterHostsApi, AdcmClusterMappingApi, AdcmJobsApi, type RequestError } from '@api';
...
const addLastStage = (state: AdcmClusterHostsWizardState) => {
...
const getProcess = createAsyncThunk(
  'adcm/clusterHostsWizard/getProcess',
  async ({ clusterId, hostId, actionId, processId }: AdcmGetProcessPayload, thunkAPI) => {
    try {
      const process = await AdcmClusterHostsApi.getHostActionWizardProcess(clusterId, hostId, actionId, processId);
      return process;
    } catch (error) {
...
const getProcessOnActionClick = createAsyncThunk(
  'adcm/clusterHostsWizard/getProcessOnActionClick',
  async ({ clusterId, hostId, actionId, processId }: AdcmGetProcessPayload, thunkAPI) => {
    try {
      const process = await AdcmClusterHostsApi.getHostActionWizardProcess(clusterId, hostId, actionId, processId);
...
const refreshProcessStages = createAsyncThunk(
  'adcm/clusterHostsWizard/refreshProcessStages',
  async ({ clusterId, hostId, actionId, processId }: AdcmGetProcessPayload, thunkAPI) => {
    try {
      const process = await AdcmClusterHostsApi.getHostActionWizardProcess(clusterId, hostId, actionId, processId);
      return process;
...
const getStep = createAsyncThunk(
  'adcm/clusterHostsWizard/getStep',
  async ({ clusterId, hostId, actionId, processId, stepId }: AdcmGetStepPayload, thunkAPI) => {
    try {
      const step = await AdcmClusterHostsApi.getHostActionWizardStep(clusterId, hostId, actionId, processId, stepId);
      return step;
    } catch (error) {
...
const getSteps = createAsyncThunk(
  'adcm/clusterHostsWizard/getSteps',
  async ({ clusterId, hostId, actionId, processId, stepIds }: AdcmGetStepsPayload, thunkAPI) => {
    try {
      ...
      const stepPromises = stepIds.map((stepId) =>
        AdcmClusterHostsApi.getHostActionWizardStep(clusterId, hostId, actionId, processId, stepId, {
...
const loadJobFromBackend = createAsyncThunk(
  'adcm/clusterHostsWizard/loadJobFromBackend',
...
const getJob = createAsyncThunk('adcm/clusterHostsWizard/getJob', async (arg: loadJobPayload, thunkAPI) => {
...
const loadSubJobLogFromBackend = createAsyncThunk(
  'adcm/clusterHostsWizard/loadSubJobLogFromBackend',
...
type GetClustersHostsWizardMappingArg = {
  clusterId: number;
};

const loadMappings = createAsyncThunk(
  'adcm/clusterHostsWizard/loadMappings',
  async ({ clusterId }: GetClustersHostsWizardMappingArg, thunkAPI) => {
    try {
...
const getMappings = createAsyncThunk(
  'adcm/clusterHostsWizard/getMappings',
  async (arg: GetClustersHostsWizardMappingArg, thunkAPI) => {
    await thunkAPI.dispatch(loadMappings(arg));
  },
);

const refreshMapping = createAsyncThunk(
  'adcm/clusterHostsWizard/mapping/refreshMapping',
  async ({ clusterId }: GetClustersHostsWizardMappingArg, _thunkAPI) => {
    const mapping = await AdcmClusterMappingApi.getMapping(clusterId);
    return mapping;
  },
);
...
const refreshMappingHosts = createAsyncThunk(
  'adcm/clusterHostsWizard/mapping/refreshMappingHosts',
  async ({ clusterId }: GetClustersHostsWizardMappingArg, _thunkAPI) => {
    const hosts = await AdcmClusterMappingApi.getMappingHosts(clusterId);
    return hosts;
  },
...
const refreshMappingComponents = createAsyncThunk(
  'adcm/clusterHostsWizard/mapping/refreshMappingComponents',
  async ({ clusterId }: GetClustersHostsWizardMappingArg, _thunkAPI) => {
    const components = await AdcmClusterMappingApi.getMappingComponents(clusterId);
    return components;
  },
...
type AdcmClusterHostsWizardState = {
...
const createInitialState = (): AdcmClusterHostsWizardState => ({
  process: null,
  steps: [],
  step: null,
  jobsData: {},
...
const clusterHostsWizardSlice = createSlice({
  name: 'adcm/clusterHostsWizard',
...
export const {
  ...
  resetJobData,
  resetJobDataByStep,
} = clusterHostsWizardSlice.actions;
export {
  getStep,
  getSteps,
  getProcess,
...
export default clusterHostsWizardSlice.reducer;

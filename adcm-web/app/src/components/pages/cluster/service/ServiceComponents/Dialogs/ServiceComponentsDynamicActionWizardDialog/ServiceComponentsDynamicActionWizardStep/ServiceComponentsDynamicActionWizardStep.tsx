...
import type { RunClusterServiceComponentDynamicActionPayload } from '@store/adcm/cluster/services/serviceComponents/serviceComponentsDynamicActionsSlice';
import ServiceComponentsDynamicActionWizardOperation from '@pages/cluster/service/ServiceComponents/Dialogs/ServiceComponentsDynamicActionWizardDialog/ServiceComponentsDynamicActionWizardOperation/ServiceComponentsDynamicActionWizardOperation';

interface ClusterServiceComponentsDynamicActionWizardStepProps {
  stageNumber: number;
  onClose: () => void;
}
...
const ServiceComponentsDynamicActionWizardStep: React.FC<ClusterServiceComponentsDynamicActionWizardStepProps> = ({
  stageNumber,
  onClose,
}: ClusterServiceComponentsDynamicActionWizardStepProps) => {
  ...
  const process = useStore((s) => s.adcm.clusterServiceComponentsWizard.process);
  const inProgress = useStore(({ adcm }) => adcm.clusterServiceComponentsWizardActions.wizardDialog.inProgress);
  ...
  const step = useStore(({ adcm }) => adcm.clusterServiceComponentsWizard.step);
  ...
  const handleSubmitStep = (stepType: string, options?: SubmitStepHandlerOptions) => {
    ...
    if (stepType === AdcmWizardStepType.LastStep) {
      ...
      const lastStepPayload: RunClusterServiceComponentDynamicActionPayload = {
        clusterId,
        serviceId,
        componentId,
        actionId,
  ...
  const handleResetStep = (stepId: number) => {
    if (!clusterId || !serviceId || !componentId || !actionId || !process || inProgress) {
      return;
    }
  ...
  const handleChangeStep = () => {
    if (clusterId && serviceId && componentId && actionId && process && currentStep && step) {
      if (isMaxStepInStage) {
        dispatch(resetStep());
      }
      ...
      dispatch(getProcess({ clusterId, serviceId, componentId, actionId, processId: process.id }));
    }
  };
...
export default ServiceComponentsDynamicActionWizardStep;

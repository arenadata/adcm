...
def remove_groups(addresses: Collection[GroupAddress]) -> list[GroupAddress]:
    ...
    config_ids = [
        group.config_id for group in existing.values() if isinstance(group, ConfigHostGroup) and group.config_id
    ]
...
def _candidate_host_ids(group: ConfigHostGroup) -> set[HostID]:
    ...
    match owner:
        ...
        case Service():
            candidates = Host.objects.filter(cluster_id=owner.cluster_id, hostcomponent__service_id=owner.pk)
        case Component():
            candidates = Host.objects.filter(cluster_id=owner.cluster_id, hostcomponent__component_id=owner.pk)
        case _:
...
def resolve_host_names(owner: HostGroupOwner, names: Collection[HostName]) -> dict[HostName, HostID]:
    ...
    if isinstance(owner, Provider):
        ...
        candidates = Host.objects.filter(cluster_id=owner.pk if isinstance(owner, Cluster) else owner.cluster_id)

...
from core.action import HcAclRule, HostSource, TaskMappingDelta
...
_GROUP_KINDS = {"config_host_group", "action_host_group"}
...
def _resolve_cluster_names(source: Iterable[HostSource], context: SelectionContext) -> dict[str | None, ClusterID]:
    ...
    names = {entry.cluster_name for entry in source if getattr(entry, "cluster_name", None) is not None}
...
def _hosts_of_groups(source: Iterable[HostSource], context: SelectionContext) -> set[HostID]:
    entries = [entry for entry in source if entry.type in _GROUP_KINDS]
    if not entries:
        return set()
...
def add_duplicates(
    ...
    for original in originals:
        if original.cluster_id in target_cluster_ids:
            held[original.cluster_id][original.pk] = original.pk

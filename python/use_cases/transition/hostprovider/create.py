...
class CreateHostprovider:
    ...
    def do(self, prototype: Prototype, name: str, description: str) -> ProviderID:
        ...
        with atomic():
            ...
            concern_id = None
            related_objects = {}
            if object_configuration_has_issue(provider, config_service=self.config_service):
...
class CreateHost:
    ...
    def do(self, hostprovider: Provider, name: str, cluster: Cluster | None, description: str = "") -> HostID:
        with atomic():
            ...
            host_concern_map = recalculate_own_concerns_on_add_hosts(host=host, config_service=self.config_service)
            ...
            # Re apply policies
            self.rbac_scenarios.re_apply_object_policy(apply_object=host.provider)
        ...
        # Send events
        self.status_scenarios.notify_about_redistributed_concerns_from_maps(added=concern_map, removed={})

        if concern_id:
            self.status_scenarios.notify_about_new_concern(concern_id=concern_id, related_objects=related_objects)

        logger.info("host #%s %s is created", host.pk, host.fqdn)

...
def create_duplicates(
    ...
    originals_with_config = [
        (original, duplicate) for original, duplicate in zip(originals, duplicates, strict=True) if original.config_id
    ]
    if originals_with_config:
        by_prototype = defaultdict(list)
        for original, duplicate in originals_with_config:
            by_prototype[original.prototype_id].append((original, duplicate))

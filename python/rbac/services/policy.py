...
def _check_objects(role: Role, objects: List[ObjectType]) -> None:
...
def policy_update(policy: Policy, **kwargs) -> Policy:
    ...
    objects = kwargs.get('object')
    _check_objects(
        role if role is not None else policy.role,
        objects if objects is not None else policy.object.all(),
    )

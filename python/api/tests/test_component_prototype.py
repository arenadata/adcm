...
class TestComponentPrototypeAPI(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def test_list(self):
        response: Response = self.client.get(path=reverse(viewname="v1:component-prototype-list"))

        self.assertEqual(len(response.data["results"]), 2)

    def test_list_filter_name(self):
        response: Response = self.client.get(
            path=reverse(viewname="v1:component-prototype-list"), data={"name": "test_prototype_2"}
        )
    ...
    def test_list_filter_bundle_id(self):
        response: Response = self.client.get(
            path=reverse(viewname="v1:component-prototype-list"),
    ...
    def test_list_ordering_display_name(self):
        response: Response = self.client.get(
            path=reverse(viewname="v1:component-prototype-list"), data={"ordering": "display_name"}
        )
    ...
    def test_list_ordering_version_order(self):
        response: Response = self.client.get(
            path=reverse(viewname="v1:component-prototype-list"), data={"ordering": "version_order"}
        )
    ...
    def test_retrieve(self):
        response: Response = self.client.get(
            path=reverse(viewname="v1:component-prototype-detail", kwargs={"prototype_pk": self.prototype_2.pk}),
        )

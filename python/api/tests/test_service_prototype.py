...
class TestServicePrototypeAPI(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def test_list(self):
        response: Response = self.client.get(path=reverse("service-prototype-list"))

        self.assertEqual(len(response.data["results"]), 2)

    def test_list_filter_name(self):
        response: Response = self.client.get(reverse("service-prototype-list"), {"name": "test_prototype_2"})
    ...
    def test_list_filter_bundle_id(self):
        response: Response = self.client.get(
            reverse("service-prototype-list"),
    ...
    def test_list_ordering_display_name(self):
        response: Response = self.client.get(reverse("service-prototype-list"), {"ordering": "display_name"})
    ...
    def test_list_ordering_version_order(self):
        response: Response = self.client.get(reverse("service-prototype-list"), {"ordering": "version_order"})
    ...
    def test_retrieve(self):
        response: Response = self.client.get(
            reverse("service-prototype-detail", kwargs={"prototype_pk": self.prototype_2.pk}),
        )
    ...
    def test_action(self):
        response: Response = self.client.get(
            reverse("service-prototype-actions", kwargs={"prototype_pk": self.prototype_1.pk}),
        )

        self.assertEqual(response.data[0]["id"], self.action.pk)

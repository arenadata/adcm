...
from core.job.types import ExecutionStatus, Job, JobParams, JobSpec, ScriptType, StateChanges, TaskMappingDelta
...
def _get_selector_for_core_object(target: CoreObjectDescriptor, owner: CoreObjectDescriptor) -> dict:
    ...
    query = model_.objects.values(**_SELECTOR_FIELDS_MAP[model_]).filter(id=target.id)  # pyright: ignore [reportArgumentType]

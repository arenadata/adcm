...
auth_model_list = [('auth', 'user'), ('auth', 'group'), ('adwp_rbac', 'role')]
...
def fill_view_role(Role, Permission):
    ...
    for model in ops_model_list:
        ...
        perm = Permission.objects.get(codename=codename, content_type__app_label='cm')
        role.permissions.add(perm)
    for (app, model) in auth_model_list:
        codename = f'view_{model}'
...
def fill_role(apps, **kwargs):
    if not RBAC:
        return
    Role = apps.get_model('adwp_rbac', 'Role')

...
class TestCHGNaming(BaseServiceCHGTestCase):
    def test_create_group_with_same_name_for_different_entities_of_same_type_success(self) -> None:
        ...
        with self.subTest("Cluster"):
            self.assertEqual(ConfigHostGroup.objects.filter(name=self.cluster_1_host_group.name).count(), 1)

            response = self.client.v2[self.cluster_2, CONFIG_GROUPS].post(
                data={"name": self.cluster_1_host_group.name, "description": "group-config-new"},
            )

            self.assertEqual(response.status_code, HTTP_201_CREATED)
            self.assertEqual(ConfigHostGroup.objects.filter(name=self.cluster_1_host_group.name).count(), 2)

        with self.subTest("Service"):
            self.assertEqual(ConfigHostGroup.objects.filter(name=self.service_1_host_group.name).count(), 1)

            response = self.client.v2[service_2, CONFIG_GROUPS].post(
                data={"name": self.service_1_host_group.name, "description": "group-config-new"}
            )

            self.assertEqual(response.status_code, HTTP_201_CREATED)
            self.assertEqual(ConfigHostGroup.objects.filter(name=self.service_1_host_group.name).count(), 2)

        with self.subTest("Component"):
            name = "component_group"
            self.assertEqual(ConfigHostGroup.objects.filter(name=name).count(), 0)
            ...
            self.assertEqual(response.status_code, HTTP_201_CREATED)
            self.assertEqual(ConfigHostGroup.objects.filter(name=name).count(), 1)
            ...
            self.assertEqual(response.status_code, HTTP_201_CREATED)
            self.assertEqual(ConfigHostGroup.objects.filter(name=name).count(), 2)
...
class TestClusterCHG(BaseClusterCHGTestCase):
    def test_list_success(self):
        ...
        self.assertEqual(response.json()["results"][0]["id"], self.cluster_1_host_group.pk)

    def test_retrieve_success(self):
        response = self.client.v2[self.cluster_1_host_group].get()

        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertEqual(response.json()["id"], self.cluster_1_host_group.pk)
    ...
    def test_create_no_permissions_fail(self):
        initial_host_group_ids = set(ConfigHostGroup.objects.values_list("id", flat=True))
        ...
        self.assertSetEqual(initial_host_group_ids, set(ConfigHostGroup.objects.values_list("id", flat=True)))
    ...
    def test_list_hosts_success(self):
        response = self.client.v2[self.cluster_1_host_group, "hosts"].get()
    ...
    def test_host_candidates(self):
        with self.subTest("Group"):
            response = self.client.v2[self.cluster_1_host_group, "host-candidates"].get()
    ...
    def test_delete_host_success(self):
        response = self.client.v2[self.cluster_1_host_group, "hosts", self.host].delete()

        self.assertEqual(response.status_code, HTTP_204_NO_CONTENT)
        self.assertEqual(self.host, Host.objects.get(id=self.host.pk))
        self.assertNotIn(self.host, self.cluster_1_host_group.hosts.all())
    ...
    def test_adcm_5199_config_description_inheritance(self):
        ...
        self.assertEqual(ConfigLog.objects.get(pk=host_group.config.current).description, "init")

        response = self.client.v2[self.cluster_1, "configs"].post(data=config_data)
        self.assertEqual(response.status_code, HTTP_201_CREATED)

        host_group.refresh_from_db()
        self.assertEqual(ConfigLog.objects.get(pk=host_group.config.current).description, config_data["description"])
    ...
    def test_filtering_success(self):
        ...
        filters = {
            "id": (self.cluster_1_host_group.pk, None, 0),
            "name": (self.cluster_1_host_group.name, self.cluster_1_host_group.name[1:-3].upper(), "wrong"),
            "description": (
                self.cluster_1_host_group.description,
                self.cluster_1_host_group.description[1:-3].upper(),
                "wrong",
            ),
        }
...
class TestServiceCHG(BaseServiceCHGTestCase):
    def test_list_success(self):
        ...
        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertEqual(response.json()["count"], 1)
        self.assertEqual(response.json()["results"][0]["id"], self.service_1_host_group.pk)

    def test_retrieve_success(self):
        response = self.client.v2[self.service_1_host_group].get()

        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertEqual(response.json()["id"], self.service_1_host_group.pk)
    ...
    def test_adcm_5285_edit_success(self):
        ...
        with self.grant_permissions(to=self.test_user, on=self.service_1, role_name="Service Administrator"):
            ...
            response = self.client.v2[host_group, "configs"].post(data=new_config)
    ...
    def test_list_hosts_success(self):
        response = self.client.v2[self.service_1_host_group, "hosts"].get()
    ...
    def test_add_not_mapped_host_fail(self):
        ...
        response = self.client.v2[self.service_1_host_group, "hosts"].post(data={"hostId": self.host_in_cluster.pk})
        ...
        self.assertEqual(self.service_1_host_group.hosts.count(), initial_hosts_count)

    def test_delete_host_success(self):
        response = self.client.v2[self.service_1_host_group, "hosts", self.host].delete()

        self.assertEqual(response.status_code, HTTP_204_NO_CONTENT)
        self.assertEqual(self.host, Host.objects.get(id=self.host.pk))
        self.assertIn(self.host, self.cluster_1_host_group.hosts.all())
        self.assertNotIn(self.host, self.service_1_host_group.hosts.all())

    def test_host_candidates_success(self):
        response = self.client.v2[self.service_1_host_group, "host-candidates"].get()
...
class TestComponentCHG(BaseServiceCHGTestCase):
    ...
    def test_list_success(self):
        ...
        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertEqual(response.json()["count"], 1)
        self.assertEqual(response.json()["results"][0]["id"], self.component_1_host_group.pk)

    def test_retrieve_success(self):
        response = self.client.v2[self.component_1_host_group].get()

        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertEqual(response.json()["id"], self.component_1_host_group.pk)
    ...
    def test_list_hosts(self):
        response = self.client.v2[self.component_1_host_group, "hosts"].get()

        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertEqual(len(response.json()), 1)
        self.assertEqual(response.json()[0]["name"], self.host.name)

    def test_add_host_config_host_group_not_found_fail(self):
        non_existing_id = self.component_1_host_group.pk + 1000
    ...
    def test_list_host_candidates_success(self):
        response = self.client.v2[self.component_1_host_group, "host-candidates"].get()
    ...
    def test_delete_host_success(self):
        response = self.client.v2[self.component_1_host_group, "hosts", self.host].delete()

        self.assertEqual(response.status_code, HTTP_204_NO_CONTENT)
        self.assertEqual(self.host, Host.objects.get(id=self.host.pk))
        self.assertIn(self.host, self.cluster_1_host_group.hosts.all())
        self.assertIn(self.host, self.service_1_host_group.hosts.all())
        self.assertNotIn(self.host, self.component_1_host_group.hosts.all())
    ...
    def test_permissions_another_object_role_retrieve_denied(self):
        ...
        with self.grant_permissions(
            ...
            with self.grant_permissions(
                ...
                response = self.client.v2[self.component_2_host_group].get()
                self.assertEqual(response.status_code, HTTP_403_FORBIDDEN)
    ...
    def test_adcm_5967_move_host_in_context_of_one_service(self):
        ...
        self.assertEqual(self.component_1_host_group.hosts.count(), 0)
        self.assertEqual(self.service_1_host_group.hosts.count(), 1)
        self.assertListEqual(list(self.service_1_host_group.hosts.all()), [self.host])
...
class TestProviderCHG(BaseAPITestCase):
    ...
    def test_retrieve_success(self):
        response = self.client.v2[self.host_group].get()
    ...
    def test_create_without_config_fail(self):
        ...
        self.assertEqual(response.status_code, HTTP_409_CONFLICT)
        self.assertEqual(ConfigHostGroup.objects.count(), initial_host_groups_count)
    ...
    def test_delete_success(self):
        response = self.client.v2[self.host_group].delete()
    ...
    def test_delete_host_success(self):
        ...
        self.assertEqual(response.status_code, HTTP_204_NO_CONTENT)
        self.assertNotIn(self.host, self.host_group.hosts.all())
...
class TestHostCandidateForConfigHostsGroups(BaseAPITestCase):
    ...
    def create_group_and_add_hosts_via_api(
        self, object_: ObjectWithCHG, name: str, hosts: Iterable[Host]
    ) -> ConfigHostGroup:
        ...
        return ConfigHostGroup.objects.get(id=group_id)

    def check_host_candidates_of_object(self, object_: ObjectWithCHG, hosts: Iterable[Host]) -> None:
    ...
    def check_host_candidates_of_group(self, group: ConfigHostGroup, hosts: Iterable[Host]) -> None:

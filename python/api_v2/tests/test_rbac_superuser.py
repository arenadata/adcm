...
class TestUserCreateEdit(BaseAPITestCase):
    ...
    def test_create_user_perm_does_not_allow_superuser_creation(self) -> None:
        response = self.request_create_user(client=self.creator_client, data=self.new_user_data | {"isSuperUser": True})
    ...
    def test_superuser_can_create_superuser(self) -> None:
        response = self.request_create_user(client=self.client, data=self.new_user_data | {"isSuperUser": True})
    ...
    def test_edit_user_perm_does_not_allow_superuser_status_change(self) -> None:
        with self.subTest("Edit Oneself"):
            response = self.request_edit_user(
                client=self.editor_client, user_id=self.editor.pk, data={"isSuperUser": True}
            )
        ...
        with self.subTest("Edit Another"):
            response = self.request_edit_user(
                client=self.editor_client, user_id=self.creator.pk, data={"isSuperUser": True}
            )
        ...
        with self.subTest("Withdraw"):
            ...
            response = self.request_edit_user(client=self.editor_client, user_id=admin.pk, data={"isSuperUser": False})
    ...
    def test_superuser_can_change_superuser_status_of_another_user(self) -> None:
        with self.subTest("Can Grant"):
            response = self.request_edit_user(client=self.client, user_id=self.editor.pk, data={"isSuperUser": True})
        ...
        with self.subTest("Can Withdraw"):
            response = self.request_edit_user(client=self.client, user_id=self.editor.pk, data={"isSuperUser": False})
    ...
    def test_superuser_cannot_withdraw_own_superuser_status(self) -> None:
        admin = User.objects.get(username="admin")
        response = self.request_edit_user(client=self.client, user_id=admin.pk, data={"isSuperUser": False})

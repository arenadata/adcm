...
class TestHostManage(TestCase):
    ...
    def test_parse_in_action_mode_success(self):
        for version, parser in get_parsers():
            with self.subTest(version):
                ...
                self.assertEqual(parsed[0].params["operation"], "add_duplicates")
                self.assertEqual(
                    parsed[0].params["source"][0],
                    {
                        "type": "component",
                        "cluster_name": "adb-one",
                        "service_name": "adb",
                ...
                self.assertEqual(
                    parsed[0].params["mapping_rules"],
                    [{"service": "adb_clusters", "component": "adcc_agent", "action": "add"}],
                )
                self.assertEqual(parsed[1].params["groups"][0]["name"], "adb-one")
                self.assertEqual(parsed[3].params["groups"][0]["description"], "per-cluster settings")
                self.assertEqual(parsed[3].params["groups"][0]["parameters"], [{"key": "adcc_agent/ram", "value": 4}])
    ...
    def test_empty_hosts_survives_the_dump_success(self):
        ...
        self.assertEqual(parsed[0].params["groups"][0]["hosts"], [])

    def test_omitted_hosts_is_absent_from_the_dump_success(self):
        parsed = self.parse(
            """
            - name: create group
              script: host_group_manage
        ...
        self.assertNotIn("hosts", parsed[0].params["groups"][0])

...
_VariantResolver: TypeAlias = Callable[[VariantParameter], tuple]
...
def _get_basic_schema(parameter: SimpleParameter | ParameterGroup, read_only: bool) -> JSONSchemaNodeDict:
    return JSONSchemaNodeDict(
        ...
        adcmMeta=ADCMMetaDict(
            ...
            isSecret=parameter.is_secret if isinstance(parameter, StringParameter | MapParameter) else False,
            activation=None,
            synchronization=None,
            stringExtra=None,
            enumExtra=None,
...
def _fill_type_specifics_to_schema_node(
    schema: JSONSchemaNodeDict, parameter: SimpleParameter, resolve_variant: _VariantResolver
) -> None:

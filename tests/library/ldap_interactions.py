...
class LDAPEntityManager:
    ...
    def create_user(self, name: str, password: str, custom_base_dn: str = None) -> str:
        ...
        self.conn.modify_s(new_dn, [(ldap.MOD_REPLACE, 'unicodePwd', [pass_utf16])])  # pylint: disable=no-member
        # activate user
        self.conn.modify_s(new_dn, [(ldap.MOD_REPLACE, 'userAccountControl', b'512')])  # pylint: disable=no-member
        self._created_records.append(new_dn)
        return new_dn

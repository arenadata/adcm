...
def generate_cluster_success_params() -> List[ParameterSet]:
    ...
    return [
        *[
            pytest.param('set_cluster', get_cluster, from_obj_func, id=f'set_cluster_multi_state_from_{from_obj_id}')
        ...
        *[
            pytest.param(
                'set_service', get_first_service, from_obj_func, id=f'set_service_multi_state_from_{from_obj_id}'
            )
            for from_obj_func, from_obj_id in ((get_first_service, 'self'), (get_first_component, 'component'))
        ],
        pytest.param(
            'set_component', get_first_component, get_first_component, id='set_component_multi_state_from_self'
        ),
        *[
            pytest.param(
                'set_first_service', get_first_service, from_obj_func, id=f'set_service_multi_state_from_{from_obj_id}'
            )
            for from_obj_func, from_obj_id in (
        ...
        *[
            pytest.param(
                ...
                get_first_component,
                from_obj_func,
                id=f'set_component_multi_state_from_{from_obj_id}',
            )
            for from_obj_func, from_obj_id in (
...
def generate_provider_success_params() -> List[ParameterSet]:
    ...
    return [
        pytest.param('set_provider', get_provider, get_provider, id='set_provider_multi_state_from_self'),
        pytest.param('set_provider', get_provider, get_host, id='set_provider_multi_state_from_host'),
        pytest.param('set_host', get_host, get_host, id='set_host_multi_state_from_self'),
    ]
...
def test_cluster_related_objects(
    ...
    _test_successful_multi_state_set(
        action_name,
        get_object_to_be_changed,
        get_action_owner_object,
        sdk_client_fs,
...
def check_cluster_related_objects_multi_state(
...
@allure.step('Check that multi_state_set affected correct ADCM objects')
def check_provider_related_objects_multi_state(
    adcm_client: ADCMClient,
    changed_adcm_objects: Collection[Union[Provider, Host]] = (),
    changed_multi_state: Optional[Set[str]] = None,
    unchanged_multi_state: Optional[Set[str]] = None,
):
    """Check that multi state changed correctly in provider related objects: Providers, Hosts"""
    changed_multi_state = changed_multi_state or {'ifeelgood!'}
    for adcm_object in changed_adcm_objects:
        _compare_multi_states(adcm_object, changed_multi_state)

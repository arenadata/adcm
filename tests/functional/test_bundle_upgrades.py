...
def test_that_check_nonexistent_cluster_upgrade(
    sdk_client_fs: ADCMClient, cluster_bundles
):
...
def test_that_check_nonexistent_hostprovider_upgrade(
...
def test_shouldnt_upgrade_upgrated_hostprovider(
    sdk_client_fs: ADCMClient, host_bundles
):
    bundle, upgrade_bundle = host_bundles
    hostprovider_bundle = sdk_client_fs.upload_from_fs(bundle)
    ...
    with allure.step("Check errors"):
        ...
        assert (
            "is more than upgrade max version" in e.value.error["desc"]
        ), e.value.error["desc"]
...
def test_upgrade_contains_strict_and_nonstrict_value(
    sdk_client_fs: ADCMClient, boundary, expected
):

...
def task_generator(action, selector):
    ...
    try:
        ...
        raise AdcmEx(
            "TASK_GENERATOR_ERROR", "action stop of service ZOOKEEPER not found"
        ) from None

    try:
        start = Action.objects.get(
            context="service", context_id=service.id, name="start"
        )
    except Prototype.DoesNotExist:
        raise AdcmEx(
...
def task_get(action, selector):
    ...
    try:
        ...
        raise AdcmEx(
            "TASK_GENERATOR_ERROR", "action stop in service Simple_service not found"
        ) from None

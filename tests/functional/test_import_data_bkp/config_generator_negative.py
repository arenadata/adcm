...
for variable in VARIABLES:
    ...
    with open("{}/config.yaml".format(d_name), "w+") as f:
        f.write(TEMPLATE_SERVICE.format(variable[1], variable[2], variable[3], variable[4], variable[5]))

for variable in VARIABLES:
    d_name = "cluster_import_check_negative/{}_{}_{}".format(variable[0], variable[3], variable[4])
    os.makedirs(d_name)
    with open("{}/config.yaml".format(d_name), "w+") as f:
        f.write(TEMPLATE_CLUSTER.format(variable[1], variable[2], variable[3], variable[4], variable[5]))

...
def prepared_cluster(client):
    ...
    return client.cluster.create(
        prototype_id=client.stack.cluster.list()[0]["id"], name=utils.random_string()
    )
...
def hostprovider(client):
    steps.upload_bundle(
        client, utils.get_data_dir(__file__, "host_bundle_on_any_level")
    )
...
def test_run_new_action_on_locked_cluster_must_throws_exception(
    ...
    with allure.step("Run first action: lock cluster"):
        ...
        client.cluster.action.run.create(
            action_id=lock_action["id"], cluster_id=cluster["id"]
        )
    with allure.step("Run second action: install"):
        with pytest.raises(coreapi.exceptions.ErrorMessage) as e:
            client.cluster.action.run.create(
                action_id=install_action["id"], cluster_id=cluster["id"]
            )
...
def test_service_in_cluster_must_be_locked_when_cluster_action_running(
    ...
    with allure.step("Create service and run action: lock cluster"):
        cluster = prepared_cluster
        service = client.cluster.service.create(
            cluster_id=cluster["id"], prototype_id=client.stack.service.list()[0]["id"]
        )
        client.cluster.action.run.create(
            action_id=get_action_by_name(client, cluster, "lock-cluster")["id"],
            cluster_id=cluster["id"],
...
def test_host_in_cluster_must_be_locked_when_cluster_action_running(
    ...
    with allure.step("Check if host is locked"):
        assert (
            client.cluster.host.read(cluster_id=cluster["id"], host_id=host["id"])[
                "state"
            ]
            == "locked"
        )
...
def test_cluster_must_be_locked_when_located_host_action_running(
    ...
    with allure.step("Create host and run action: action locker"):
        client.cluster.host.create(
            cluster_id=prepared_cluster["id"], host_id=host["id"]
        )
        client.host.action.run.create(
            action_id=filter_action_by_name(
                client.host.action.list(host_id=host["id"]), "action-locker"
    ...
    with allure.step("Check if host and cluster are locked"):
        assert (
            client.cluster.host.read(
                cluster_id=prepared_cluster["id"], host_id=host["id"]
            )["state"]
            == "locked"
        )
        assert (
            client.cluster.read(cluster_id=prepared_cluster["id"])["state"] == "locked"
        )
...
def test_cluster_service_locked_when_located_host_action_running(
    ...
    with allure.step("Check if host, cluster and service are locked"):
        ...
        assert (
            client.cluster.service.read(
                cluster_id=prepared_cluster["id"], service_id=service["id"]
            )["state"]
            == "locked"
        )
...
def test_run_service_action_locked_all_objects_in_cluster(
    ...
    with allure.step("Run action: service lock"):
        client.cluster.service.action.run.create(
            action_id=filter_action_by_name(
                client.cluster.service.action.list(
                    cluster_id=prepared_cluster["id"], service_id=service["id"]
                ),
                "service-lock",
            )[0]["id"],
...
def test_cluster_should_be_unlocked_when_ansible_task_killed(client, prepared_cluster):
    with allure.step("Run cluster action: lock terminate"):
        task = client.cluster.action.run.create(
            action_id=get_action_by_name(client, prepared_cluster, "lock-terminate")[
                "id"
            ],
            cluster_id=prepared_cluster["id"],
        )
    with allure.step("Check if cluster is locked and then terminate_failed"):
        ...
        assert (
            client.cluster.read(cluster_id=prepared_cluster["id"])["state"]
            == "terminate_failed"
        )
...
def test_hostprovider_must_be_unlocked_when_his_task_finished(client, hostprovider):
    with allure.step("Run action: action locker and create hostprovider"):
        action_id = filter_action_by_name(
            client.provider.action.list(provider_id=hostprovider["id"]), "action-locker"
        )[0]["id"]
        task = client.provider.action.run.create(
            action_id=action_id, provider_id=hostprovider["id"]
        )
    with allure.step("Check if provider is locked and then created"):
        ...
        wait_until(client, task)
        assert (
            client.provider.read(provider_id=hostprovider["id"])["state"] == "created"
        )

...
class TestClusterServiceConfig:
    ...
    def test_shouldnt_create_service_config_when_config_doesnt_have_one_req_key(
        self, client
    ):
        cluster = steps.create_cluster(client)
        config_wo_required_key = {
            "ssh-key": "TItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAA",
            "zoo.cfg": {
    ...
    def test_shouldnt_create_service_config_when_integer_param_bigger_than_boundary(
        self, client
    ):
        cluster = steps.create_cluster(client)
        config_int_bigger_boundary = {
            "ssh-key": "TItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAA",
            "zoo.cfg": {
    ...
    def test_shouldnt_create_service_config_when_integer_param_less_than_boundary(
        self, client
    ):
        cluster = steps.create_cluster(client)
        config_int_less_boundary = {
            "ssh-key": "TItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAA",
            "zoo.cfg": {
    ...
    def test_shouldnt_create_service_config_when_float_param_bigger_than_boundary(
        self, client
    ):
        cluster = steps.create_cluster(client)
        config_float_bigger_boundary = {
            "ssh-key": "TItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAA",
            "zoo.cfg": {
    ...
    def test_when_delete_host_all_children_cannot_be_deleted(self, client):
        ...
        with allure.step("Create hostcomponent"):
            steps.create_hostcomponent_in_cluster(
...
class TestClusterServiceConfigHistory:
    def test_config_history_url_must_point_to_the_service_config(self, client):
        ...
        with allure.step("Check config history"):
            for conf in history:
                assert (
                    "cluster/{0}/service/".format(cluster["id"]) in conf["url"]
                ) is True
        steps.delete_all_data(client)
    ...
    def test_get_config_from_nonexistant_cluster_service(self, client):
        cluster = steps.create_cluster(client)
        with pytest.raises(coreapi.exceptions.ErrorMessage) as e:
            client.cluster.service.config.list(
                cluster_id=cluster["id"], service_id=random.randint(100, 500)
            )
...
class TestClusterConfig:
    def test_config_history_url_must_point_to_the_cluster_config(self, client):
        ...
        with allure.step("Check config history"):
            for conf in history:
                assert (
                    "api/v1/cluster/{0}/config/".format(cluster["id"]) in conf["url"]
                ) is True
        steps.delete_all_data(client)
    ...
    def test_shouldnt_create_service_config_when_config_is_number(
        ...
        with allure.step("Try to create config from number"):
            with pytest.raises(coreapi.exceptions.ErrorMessage) as e:
                client.cluster.config.history.create(
                    cluster_id=cluster["id"], config=random.randint(0, 9)
                )
    ...
    def test_shouldnt_create_config_with_unknown_key(self, client):
        ...
        with allure.step("Try to create config with unknown key"):
            with pytest.raises(coreapi.exceptions.ErrorMessage) as e:
                client.cluster.config.history.create(
                    cluster_id=cluster["id"], config=config
                )
    ...
    def test_shouldnt_create_config_when_try_to_put_map_in_option(self, client):
        ...
        with allure.step("Try to create config with map in flat key"):
            with pytest.raises(coreapi.exceptions.ErrorMessage) as e:
                client.cluster.config.history.create(
                    cluster_id=cluster["id"], config=config_with_deep_depth
                )
    ...
    def test_check_that_file_field_put_correct_data_in_file_inside_docker(self, client):
        ...
        with allure.step("Create config history"):
            client.cluster.config.history.create(
                cluster_id=cluster["id"], config=config_data
            )

...
def repr_template(event_type, obj_type, obj_id, dtype=None, value=None):
    return {
        ...
        "object": {
            "type": obj_type,
            "id": obj_id,
            "details": {"type": dtype, "value": value},
        },
    }
...
def provider(sdk_client_fs, name=utils.random_string()):
    return sdk_client_fs.upload_from_fs(
        os.path.join(DATADIR, "hostprovider")
    ).provider_create(name=name)
...
def test_event_when_host_added_to_cluster(sdk_client_fs, ws):
    ...
    with allure.step("Check host"):
        assert_events(
            ws, repr_template("add", "host", hst.host_id, "cluster", str(cl.cluster_id))
        )
...
def test_event_when_add_service(sdk_client_fs, ws):
    ...
    assert_events(
        ws, repr_template("add", "service", obj.id, "cluster", str(obj.cluster_id))
    )


@pytest.mark.parametrize(("case", "action_name", "expected"), cluster_actions)
def test_events_when_cluster_action_(
    case, action_name, expected, ws, cluster_with_svc_and_host
):

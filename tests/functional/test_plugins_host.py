...
def test_create_one_host(second_p: Provider):
    ...
    with allure.step("Run action create host"):
        second_p.action(name="create_host").run(
            config_diff={"fqdn": HOSTNAME}
        ).try_wait()
...
def test_create_multi_host_and_delete_one(first_p: Provider, third_p: Provider):
    ...
    with allure.step("Create two host from first providers"):
        first_p.action(name="create_host").run(
            config_diff={"fqdn": "one_one"}
        ).try_wait()
        first_p.action(name="create_host").run(
            config_diff={"fqdn": "one_two"}
        ).try_wait()
    with allure.step("Create one host from third provider"):
        third_p.action(name="create_host").run(
            config_diff={"fqdn": "three_one"}
        ).try_wait()
...
def test_check_host_lock_during_operations(forth_p: Provider):
    ...
    with allure.step("Create host first host on provider"):
        forth_p.action(name="create_host").run(
            config_diff={"fqdn": "forth_one"}
        ).try_wait()
    with allure.step("Run job that creates the second host on provider"):
        job = forth_p.action(name="create_host").run(
            config={"fqdn": "forth_two", "sleep": 2}
        )
    ...
    with allure.step(
        'Check that host under action is "locked", while other host is "created"'
    ):
        forth_one_h.reread()
        forth_two_h.reread()
        assert forth_one_h.state == "locked"

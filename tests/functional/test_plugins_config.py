...
ActionSuffix = Literal['all', 'multijob', 'host']


PROVIDER_UNITS = ('provider', 'host')
ProviderUnitLiteral = Literal['provider', 'host']

# !===== FIXTURES =====!


@pytest.fixture()
def initial_unit_config() -> dict:
    """
...
@pytest.fixture()
def initial_clusters_units_config(initial_unit_config) -> dict:
    """
    ...
    return {
        cluster_name: {
            'config': copy.deepcopy(initial_unit_config),
            'services': {
                service_name: {
                    'config': copy.deepcopy(initial_unit_config),
                    'components': {
...
@pytest.fixture()
def cluster_bundle(sdk_client_fs: ADCMClient, initial_clusters_units_config):
    """
    Load bundle, create clusters with services from config
    """
    bundle = sdk_client_fs.upload_from_fs(get_data_dir(__file__, "cluster"))
    for cluster_name, cluster_dict in initial_clusters_units_config.items():
...
@pytest.fixture()
def provider_bundle(sdk_client_fs: ADCMClient, initial_providers_units_config):
    """
    Load bundle, create providers with hosts from config
    """
    bundle = sdk_client_fs.upload_from_fs(get_data_dir(__file__, "provider"))
    for name in initial_providers_units_config:
        provider = bundle.provider_create(name)
        for fqdn in initial_providers_units_config[name]['hosts']:
            provider.host_create(fqdn=fqdn)
    return bundle


@pytest.fixture()
def initial_providers_units_config(initial_unit_config) -> dict:
    """
    Return dict with default configuration info
    ...
    return {
        provider_name: {
            ...
            'hosts': {
                ...
                f'{provider_name}_{host_suffix}': {'config': copy.deepcopy(initial_unit_config)}
...
@pytest.fixture()
def correct_initial_clusters_units_config(
    cluster_bundle: Bundle, initial_clusters_units_config
) -> dict:
    """
    Asserts that initial clusters configuration is correct and returns initial_config
    """
    assert_clusters_units_config_is_correct(cluster_bundle, initial_clusters_units_config)
    return initial_clusters_units_config


@pytest.fixture()
def correct_initial_providers_units_config(
    provider_bundle: Bundle, initial_providers_units_config
) -> dict:
    """
    Asserts that initial providers configuration is correct and returns initial_config
    """
    assert_providers_units_config_is_correct(provider_bundle, initial_providers_units_config)
    return initial_providers_units_config
...
def assert_providers_units_config_is_correct(bundle: Bundle, providers_expected_config: dict):
    """
...
def assert_clusters_units_config_is_correct(bundle: Bundle, clusters_expected_config: dict):
    """
...
def test_multijob(
    ...
    correct_initial_clusters_units_config,
    expected_unit_config,
    action_owner: ClusterUnitLiteral,
    change_subject: ClusterUnitLiteral,
):
    """Check multijob tasks change configuration correctly"""
    current_expected_state = correct_initial_clusters_units_config
    changer = ClusterBundleConfigChanger(
        cluster_bundle, current_expected_state, expected_unit_config
    )
    ...
    with allure.step("Check multijob action result"):
        assert_clusters_units_config_is_correct(cluster_bundle, current_expected_state)
...
def test_another_service_from_service_by_name(
    ...
    with allure.step('Check another service from service by name'):
        ...
        run_service_action_and_assert_result(one, f'service_name_{another.name}_all', 'failed')
        assert_clusters_units_config_is_correct(cluster_bundle, current_expected_state)
...
def test_provider_bundle_multijob(
    ...
    correct_initial_providers_units_config,
    expected_unit_config,
    action_owner: str,
    change_subject: str,
):
    """Test multijob action changes config of provider bundle parts"""
    current_expected_state = correct_initial_providers_units_config
    changer = ProviderBundleConfigChanger(
        provider_bundle, current_expected_state, expected_unit_config
    )
    change_func = getattr(changer, f'change_{change_subject}_config_from_{action_owner}')
    change_func('multijob')
    with allure.step("Check multijob actions changed configuration"):
        assert_providers_units_config_is_correct(provider_bundle, current_expected_state)
...
def run_action(
    ...
    task = action.run(config=config or {})
    wait_for_task_and_assert_result(task, 'success', action_name=action_name)
...
class ClusterBundleConfigChanger:
    ...
    def change_cluster_config_from_cluster(self, action_suffix: ActionSuffix):
        ...
        with allure.step(f"Run {action_name} action on cluster {cluster.name} to configure itself"):
            ...
            self.change_cluster_config(cluster.name)
    ...
    def change_service_config_from_cluster(self, action_suffix: ActionSuffix):
        ...
        with allure.step(
            ...
            run_action(cluster, action_name)
            self.change_service_config(cluster.name, service.name)
    ...
    def change_cluster_config(self, cluster_name: str):
        self.clusters_state[cluster_name]['config'] = self.expected_state

    def change_service_config(self, cluster_name: str, service_name: str):
    ...
    def change_component_config(
        ...
        service_dict = self.clusters_state[cluster_name]['services'][service_name]
        service_dict['components'][component_name]['config'] = self.expected_state
...
class ProviderBundleConfigChanger:
    ...
    def change_provider_config_from_provider(self, action_suffix: ActionSuffix):
        ...
        with allure.step(f'Run "{action_name}" on {provider.name} provider to configure itself'):
            ...
            self._change_provider_config(provider.name)
    ...
    def change_host_config_from_provider(self, action_suffix: ActionSuffix):
        ...
        with allure.step(
            ...
            self._change_host_config(provider.name, host.fqdn)
    ...
    def _change_provider_config(self, provider_name: str):
        self.providers_state[provider_name]['config'] = self.expected_state

    def _change_host_config(self, provider_name: str, host_fqdn: str):

...
class BusinessRole(NamedTuple):
    ...
    check_denied: Callable | None = None
...
class BusinessRoles(Enum):
    ...
    ADD_SERVICE = BusinessRole(
        "Add service",
        methodcaller("service_add", name="new_service"),
        Deny.AddServiceToCluster,
    )
    REMOVE_SERVICE = BusinessRole(
        "Remove service",
        lambda x, *args: x.service_delete(*args),
        Deny.RemoveServiceFromCluster,
    )
    ...
    CREATE_HOST = BusinessRole(
        "Create host",
        lambda x: x.host_create(fqdn=f"new-host-{random_string(5)}"),
        Deny.CreateHost,
    )
...
def use_role(role: BusinessRoles | Collection[BusinessRoles]):
...
def create_policy(
    sdk_client,
    permission: BusinessRoles | list[BusinessRoles],
    objects: list,
    users: list[User],
    groups: list[Group],
...
def is_allowed(
    base_object: BaseAPIObject | ADCMClient,
    business_role: BusinessRole | BusinessRoles,
    *args,
...
def check_mm_change_is_denied(
    ...
    denial_method: BusinessRoles | BusinessRole,
...
def check_mm_change_is_allowed(
    obj: Service | Component | Host,
    allow_method: BusinessRoles | BusinessRole,
    user_client: ADCMClient,
    new_mm_value: str = MM_IS_ON,
):
    """

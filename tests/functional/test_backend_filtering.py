...
def one_provider(provider_bundles: ADCMClient):
    return provider_bundles.bundle(name="provider15").provider_create(
...
def one_host(provider_bundles: ADCMClient):
    provider = provider_bundles.bundle(name="provider42").provider_create(
        name="For one Host"
    )
...
def test_coreapi_schema(sdk_client_fs: ADCMClient, TestedClass):
    ...
    with allure.step(
        f"Check if filters are acceptable for coreapi {TestedClass.__name__}"
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            lazy_fixture("provider_bundles"), HostPrototypeList, id="Host Prototype"
        ),
        ...
        pytest.param(lazy_fixture("host_with_jobs"), TaskList, id="Task"),
        pytest.param(lazy_fixture("host_with_jobs"), JobList, id="Job"),
    ],
)
...
def test_filter(
    sdk_client: ADCMClient, TestedClass, TestedListClass, search_args, expected_args
):
    """Scenario:

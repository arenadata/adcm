...
for variable in VARIABLES:
    ...
    with open("{}/import/config.yaml".format(d_name), "w+") as f:
        f.write(
            TEMPLATE_SERVICE.format(variable[1], variable[2], variable[3], variable[4])
        )
    with open("{}/export/config.yaml".format(d_name), "w+") as f:
        f.write(TEMPLATE_EXPORT_SERVICE.format(variable[5]))

for variable in VARIABLES:
    d_name = "cluster_import_check_negative/{}_{}_{}".format(
        variable[0], variable[3], variable[4]
    )
    export_dir = d_name + "/export"
    import_dir = d_name + "/import"
    ...
    with open("{}/import/config.yaml".format(d_name), "w+") as f:
        f.write(
            TEMPLATE_CLUSTER.format(variable[1], variable[2], variable[3], variable[4])
        )
    with open("{}/export/config.yaml".format(d_name), "w+") as f:

...
def test_check_cluster_list_page_with_cluster_creating(app_fs, auth_to_adcm, bundle_archive):
    ...
    cluster_params = {"bundle": f"test_cluster 1.5 {edition}", "description": "Test", "state": "created"}
    ...
    cluster_page.download_cluster(bundle_archive, cluster_params['description'],
    ...
    with allure.step("Check uploaded cluster"):
        ...
        assert cluster_params['description'] == uploaded_cluster['description'], \
            f"Cluster description should be {cluster_params['description']} and not {uploaded_cluster['description']}"
...
def test_check_cluster_list_page_pagination(sdk_client_fs: ADCMClient, app_fs, auth_to_adcm):
    ...
    with allure.step("Check pagination"):
        with cluster_page.wait_page_scroll():
            cluster_page.click_second_page()
        assert len(cluster_page.get_all_cluster_rows()) == params["second_page_cluster_amount"], \
            f"Second page should contains {params['second_page_cluster_amount']}"
        with cluster_page.wait_page_scroll():
            cluster_page.click_first_page()
        assert len(cluster_page.get_all_cluster_rows()) == params["fist_page_cluster_amount"], \
            f"First page should contains {params['fist_page_cluster_amount']}"
        with cluster_page.wait_page_scroll():
            cluster_page.click_next_page()
        assert len(cluster_page.get_all_cluster_rows()) == params["second_page_cluster_amount"], \
            f"Next page should contains {params['second_page_cluster_amount']}"
        with cluster_page.wait_page_scroll():
            cluster_page.click_previous_page()
        assert len(cluster_page.get_all_cluster_rows()) == params["fist_page_cluster_amount"], \

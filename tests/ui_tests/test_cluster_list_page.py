...
@pytest.mark.parametrize("bundle_archive",
                         [
                             pytest.param(utils.get_data_dir(__file__, "cluster_community"),
                                          id="community"),
                             pytest.param(utils.get_data_dir(__file__, "cluster_enterprise"),
                                          id="enterprise")
                         ],
                         indirect=True)
def test_check_cluster_list_page_with_cluster_creating(app_fs, auth_to_adcm, bundle_archive):
    ...
    cluster_params = {"bundle": f"test_cluster 1.5 {edition}",
                      "description": "Test", "state": "created"}

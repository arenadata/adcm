...
class ActionPage(BasePage):
    ...
    def open_run_action_popup(self):
        self._wait_element_present(
            ActionPageLocators.action_run_button, timer=15
        ).click()
    ...
    def run_action(self, is_verbose: bool = False):
        with allure.step(
            f'Run action {"with" if is_verbose else "without"} verbose checkbox'
        ):
            self.open_run_action_popup()
            if is_verbose:
                self.driver.find_element(
                    *ActionPageLocators.ActionRunPopup.verbose_chbx
                ).click()
